...
const lookup = {
  ...
  const encode = (inputString)=>{
      ...
      let encodedArr = codeArr.map((codeArrChar)=>{
          let index = lookupValues.findIndex((element)=>
            element=== codeArrChar)
        return lookupKeys[index]

        // for(let key in lookupKeys){
...
const logIn = ()=>{
    let loginPassword = loginInputPasswordField.value;
    if(loginPassword  === decode(passwordStoredInDb)){
...
const checkPasswordStrength =()=>{
    if(signupInputPasswordField.value.length < 5){
        ...
        passwordStrength.innerText = 'Password looks Weak!!(Minimum 8 characters)'
        passwordStrength.style.color = 'red'
      } else if (signupInputPasswordField.value.length < 8) {
        passwordStrength.style.display = 'block'
        passwordStrength.innerText = 'Looks Moderate!! (Password should be a mix of special characters, letters and numbers)'
